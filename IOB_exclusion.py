...
with open(train_IOB) as file:
	for line in file:
		...
		if len(w)>0:
			...
			elif w[1] == 'B-movie.name' or w[1]=='I-movie.name':
				file_IB.write(w[0]+"\t"+w[1]+"-"+w[0]+"\n")
			else:
...
with open(path+"result_auth.txt") as file:
	...
	for line in file:
		...
		if len(w)>2:
			#common line
			tmp = w[3].split('-')
			if tmp[0]=='I' or tmp[0]=='B':
				to_print = tmp[0]+"-"+tmp[1]
				tmp_str.append(to_print)
			else:
				tmp_str.append("O")
		elif len(w)==2:
			tmp_str.append('\n')
