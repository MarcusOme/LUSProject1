...
with open(train_IOB) as text:
	for line in text:
		...
		if(len(words) > 0):
			...
			lexer.append(words[1]+"--"+words[0])
			converter.append(words)
			unigrams[words[1]+"--"+words[0]] += 1
			bigrams[words[1]+"--"+words[0], words[0]] += 1
			labels.append(words[1]+"--"+words[0])
			tags_list.append(words[1]+"--"+words[0])
...
for elem, val in bigrams.items():
	...
	#p = -math.log(val/float(unigrams[elem[0]]))	#for the classiv version
	if val==1:
		p = 1	#because I want that elements that appears only once have lower probab -> cutoff lower appearance
	else:
		p = -math.log(float(val)/len(labels))	#this is the probability for the cutoff
	probab[elem] = p
...
with open(train_IOB) as file:
	...
	for line in file:
		w = line.split()
		if len(w)>1:
			w[0] = w[0].replace("'", "%")	#not present in original
			final_str += w[1]+"--"+w[0]+"\t"	#remove second part
		else:
			final_str += "\n"
